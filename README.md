...
The %rbp (stack frame pointer) and %rsp (stack pointer) works as follows. During the parsing and building of AST, we will know how much space need to be allocated for local variables. That is because the `localoffset` in `gen.c` keeps track of spaces when new locals come in. For example, in `input25.c`, we have 3 local variables. At the beginning, %rbp will have its previous value pushed, it will be the stack frame pointer (see figure in[stack frame layout on x86-64](https://eli.thegreenplace.net/2011/09/06/stack-frame-layout-on-x86-64/)). %rbp will then be passed to %rsp, making %rsp at the address of stack frame. A space of 12 bytes will be generated by moving %rsp down that much of address. Then, when those local identifiers are accessed, they are accessed by the relative address to %rbp. The %rsp is mainly for dynamically allocating spaces for stacks. When the function ends, %rsp will add back 12 bytes, backing to the stack frame address. (these are done in function preamble where stackoffset is aligned to be a multiple of 16, for %rsp and postamble).  
